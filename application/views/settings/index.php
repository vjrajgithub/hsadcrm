...
$(document).ready(function() {
    ...
    $('.save-category-settings').click(function() {
        ...
        Object.keys(formData).forEach(function(id) {
            promises.push(
                $.post('<?= base_url('settings/quick_update') ?>', {
                    id: id,
                    value: formData[id]
                })
            );
    ...
    $('#addSettingForm').submit(function(e) {
        ...
        $.post('<?= base_url('settings/add') ?>', $(this).serialize())
    ...
    $('#editSettingForm').submit(function(e) {
        ...
        $.post('<?= base_url('settings/edit') ?>/' + id, $(this).serialize())
            .done(function(response) {
                const result = JSON.parse(response);
                if (result.success) {
    ...
    $('.delete-setting').click(function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.post('<?= base_url('settings/delete') ?>/' + id)
                    .done(function(response) {
                        const result = JSON.parse(response);
                        if (result.success) {
